...
function FeaturedProducts() {
    ...
    const movePrevious = () => {
        myRef.current?.classList.add("-translate-y-20")
    }
    const moveNext = () => {
        myRef.current?.classList.add("translate-y-20")
    }
    ...
    return (
        <div className='md:h-[50vh]   featured w-[100%]'>
            <div className=''>
                <h1 className='font-bold  text-black text-center'>Sample products</h1>
            </div>
            <div className='h-[100%] flex flex-row items-center'>
                {/* <BsFillArrowRightCircleFill onClick={() => moveNext()} className='text-[#1D1D43]  md:block hi z-20 text-[5.3rem]  hover:text-[#33334D]' /> */}
                <div ref={myRef} className=' mx-auto w-[95%]   items-center mb-3  h-[100%] flex space-x-2  rounded flex-col md:flex-row justify-between'>
                    {products2.map((ft, index) => (
                        <div onClick={() => dispatch(selectedPost(ft))} className='h-[80%] w-[100%]  md:w-[30%] flex  hover:scale-100 mt-10  mydiv hover:shadow-lg rounded flex-col space-y-5   border p-4 rounded' key={index}>
                ...
                {/* <BsFillArrowLeftCircleFill onClick={() => movePrevious()} className='text-[#1D1D43] z-20 text-[5.3rem]  hover:text-[#33334D]' /> */}
            </div>
        </div>
    )
}
